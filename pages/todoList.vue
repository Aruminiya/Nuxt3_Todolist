      ...
      <div class="container mx-auto">
            ...
            <section class="py-10 px-5">
                <div class="flex">
                    <div class="relative w-full">
                        ...
                        @click="toPostTodo()"
                        type="button"
                        class="absolute border-2 border-mataWallBlack top-0 end-0 p-2.5 h-full text-sm font-medium text-white bg-mataWallBlue rounded-e-lg">
                           <span class="m-2">+</span>
                        </button>
                    </div>
                </div>
            ...
            <section class="py-10 px-5">
                <div class="todoList border-2 border-mataWallBlack bg-metaWallWhite rounded-lg overflow-hidden">
                    ...
                    <!-- 待辦事項區塊 -->
                    <div class="todos">
                        <h2 class="text-3xl text-center text-mataWallGray-300 my-10" v-if="todoMode.length===0">沒有任何喔資料！</h2>
                        <div v-for="todo in todoMode" :key="todo.id">
                                <div class="todo flex flex-row-reverse w-full text-xl p-5">
                                <div class="flex-none">
                                    <button v-if="editId !== todo.id" class="mx-2" @click="editId = todo.id;editData.content = todo.content">編輯</button>
                                    <button v-else class="mx-2" @click="toEditTodo(todo)">完成</button>
                                    <button class="mx-2">刪除</button>
                                </div>
                                <div class="flex-1">
                                    <input :id="todo.id" class="checkBox mx-2" type="checkbox" name="todo" :checked="todo.isChecked" @click="toCheckTodo(todo)">
                                    <label v-if="editId !== todo.id" :for="todo.id">{{todo.content}}</label>
                                    <input v-else v-model="editData.content" class="border-2 border-mataWallBlack rounded px-3" type="text" >
                                </div>
                            </div>
                            <hr />
                        </div>
...
export default {
    ...
    methods:{
        ...mapActions(todosStore, ['getTodos','postTodos','editTodos']),
        toPostTodo(){
            if(this.postData.content !== ""){
                this.postTodos(this.postData)
            }
        },
        toEditTodo(todo){
            this.editId = '';
            this.editTodos(this.editData,todo.id)
        },
        toCheckTodo(todo){
            this.editTodos({content:todo.content,isChecked:!todo.isChecked},todo.id)
        }
    },
